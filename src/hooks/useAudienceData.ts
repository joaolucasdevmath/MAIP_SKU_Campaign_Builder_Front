...
export function useAudienceData(): UseAudienceDataResult {
  ...
  const runAudienceFlow = async (campaignPayload: CampaignDataPayload) => {
    ...
    try {
      console.log(
        '[useAudienceData] Payload enviado para API:',
        JSON.stringify(campaignPayload, null, 2)
      );
      const campaignRes = await axiosInstance.post(
        endpoints.briefing.campaignData,
        campaignPayload
      );
      console.log(
        '[useAudienceData] Resposta completa da API:',
        JSON.stringify(campaignRes.data, null, 2)
      );

      if (campaignRes.data?.success === false) {
        let errorMsg = campaignRes.data?.errorMessage || 'Erro na API';
        if (
          errorMsg.includes('invalid literal for int()') ||
          errorMsg.includes('falha ao executar')
        ) {
          errorMsg =
            'Erro ao executar a query: A consulta não pôde ser processada. Verifique os filtros e tente novamente.';
        } else if (errorMsg.includes('Não foi possível executar')) {
          errorMsg =
            'Erro ao contar audiência: A query falhou na execução. Revise os parâmetros da segmentação.';
        }
        setError(errorMsg);
      } else if (campaignRes.data?.data) {
        setData(campaignRes.data.data);
        console.log(
          '[useAudienceData] Dados definidos:',
          JSON.stringify(campaignRes.data.data, null, 2)
        );
      } else {
        console.warn(
          '[useAudienceData] Resposta da API sem dados úteis:',
          JSON.stringify(campaignRes.data, null, 2)
        );
      ...
      if (
        ...
        errorMsg =
          'Erro ao processar resposta da API: Problema na execução da query. Tente ajustar os filtros.';
