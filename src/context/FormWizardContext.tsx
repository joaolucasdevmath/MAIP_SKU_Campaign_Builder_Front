...
type FormWizardAction = { type: 'UPDATE'; payload: Partial<CampaignData> } | { type: 'RESET' };
...
export function FormWizardProvider({ children }: { children: ReactNode }) {
  ...
  const contextValue = useMemo(
    () => ({ state, dispatch, updateCampaignData, resetCampaignData, role, setRole }),
    [state, dispatch, role]
  );

  useEffect(() => {
    if (isInitialMount.current) {
      const stored = sessionStorage.getItem(`formWizardData_${role}`);
      if (stored) {
        try {
  ...
  useEffect(() => {
    if (!isInitialMount.current) {
      sessionStorage.setItem(`formWizardData_${role}`, JSON.stringify(state));
    }
  }, [state, role]);

  return <FormWizardContext.Provider value={contextValue}>{children}</FormWizardContext.Provider>;
}
