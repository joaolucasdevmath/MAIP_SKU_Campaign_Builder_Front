...
function getQuantityFieldsSchema(
  channelList: Array<{ label: string }>
...
export const basicInfoSchema = z
  .object({
    ...
    campaign_type: z.union([
      z.string().min(1, 'Tipo obrigatório'),
      z.array(z.string()).min(1, 'Tipo obrigatório'),
    ]),
    channel: z.union([
      z.string().min(1, 'Canal obrigatório'),
      z.array(z.string()).min(1, 'Canal obrigatório'),
    ]),
...
export const legacyBasicInfoSchema = z.object({
  ...
  offer: z
    ...
    .min(1, 'Obrigatório')
    .describe('Oferta(s)')
    .describe('Descreva as ofertas desta campanha para histórico e referência futura.'),
  ...
  quantity: z
    .string({ required_error: 'Obrigatório' })
    .min(1, 'Obrigatório')
    .describe('Quantidade de Disparos por Canal'),
